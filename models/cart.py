...
class Cart:
    ...
    def listar_itens(user_id):
        linhas = query("SELECT * FROM carrinho_itens WHERE id = ?",(user_id,))
        itens = []
        for linha in linhas:
    ...
    def contar_itens(user_id):
        row = query("SELECT SUM(quantidade) as total FROM carrrinho_itens WHERE id = ?", (user_id,), one=True,)
    ...
    def adicionar(user_id, produto_id, quantidade):
        ...
        if existente:
            ...
            execute("UPDATE quantidade FROM carrinho_itens WHERE id = ?", (nova_qntd, existente["id"]),)
