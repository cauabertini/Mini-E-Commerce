...
class Order:
    ...
    def _montar(pedido_row, com_cliente=False):
        ...
        if com_cliente:
            ...
            pedido["cliente"] =  {"nome": cliente.nome if cliente else "Cliente"}
        return pedido
    ...
    def criar(user_id, itens_carrinho, subtotal, taxa_entrega, total, endereco=None):
        ...
        for item in itens_carrinho:
            execute("""INSERT INTO pedidos_itens (pedido_id, produto_id, quantidade, preco_unitario)
                    VALUES(?,?,?,?)
    ...
    @staticmethod
    def vendas_dia():
        row = query("""SELECT SUM(total) as total FROM pedidos 
